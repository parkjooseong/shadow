...
let providers: { id: string; configured: boolean; connected: boolean; conflicts?: { id: string; title: string; reason: string }[] }[];
...
describe('account workflows', () => {
  ...
  it('publishes only the chosen trimmed share title after explicit consent', async () => {
    ...
    expect(api).toHaveBeenCalledWith('/api/shares', 'POST', { title: '다음 주 일정', state: initial() });
  });
