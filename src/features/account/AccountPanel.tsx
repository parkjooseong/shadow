...
import { AccountSecurity, PasswordRecovery, sessionChanged } from './AccountSecurity';
import { CloudSyncSettings, ProviderAutoSync, type Automation } from './AutoSyncSettings';
...
interface Connection { id: string; configured: boolean; connected: boolean; recoveryRequired?: boolean; recoveryMessage?: string | null; lastSyncedAt?: string | null; configurationError?: string | null; lastError?: string | null; automation?: Automation; conflicts?: { id: string; title: string; reason: string }[] }
...
export function AccountPanel({ onClose }: { onClose: () => void }) {
  ...
  const accountApi = useCallback(<T,>(path: string, method = 'GET', body?: unknown) => api<T>(path, method, body, { accountId: accountId.current }), []);
  ...
  const [scheduler, setScheduler] = useState<{ running: boolean; error: string | null }>();
  ...
  const refresh = useCallback(async (isActive: () => boolean = () => true) => {
    const results = await Promise.allSettled([
      accountApi<CloudState>('/api/state'), accountApi<{ shares: Share[] }>('/api/shares'), accountApi<{ providers: Connection[]; scheduler?: { running: boolean; error: string | null } }>('/api/integrations'),
    ]);
    if (!isActive()) return;
    ...
    if (connections.status === 'fulfilled') { setProviders(connections.value.providers); setScheduler(connections.value.scheduler); }
  ...
  useEffect(() => {
    ...
    void api<{ user: User | null }>('/api/auth/me').then(async (result) => {
      if (!active) return;
      accountId.current = result.user?.id;
      setUser(result.user);
  ...
  const upload = () => void run(async () => {
    ...
    try {
      result = await accountApi<{ revision: number }>('/api/state', 'PUT', { state, revision: remote.revision });
    } catch (cause) {
  ...
  const pull = () => void run(async () => {
    const latest = validateCloudState(await accountApi<CloudState>('/api/state'));
    setRemote(latest);
  ...
  const sync = (provider: string) => void run(async () => {
    ...
    const result = await accountApi<{ imported: number; exported: number; deleted: number; conflicts: { id: string; title: string; reason: string }[]; warnings: string[] }>(`/api/integrations/${provider}/sync`, 'POST', {});
    await refresh();
  ...
  return <SidePanel titleId="account-title" className="type-panel" onClose={onClose}>
    ...
    {!user ? <>
      ...
      <button className="button ghost" disabled={busy || checkingSession} onClick={() => void run(async () => {
        try { await accountApi('/api/auth/logout', 'POST', {}); } catch (cause) { if (!(cause instanceof ApiError) || cause.status !== 401) throw cause; }
        setUser(null); setMode('login'); setRemote(undefined); setProviders([]); setShares([]); setShareTitle('SHADOW 일정 공유');
        sessionChanged();
        setMessage('로그아웃했습니다. 이 브라우저의 데이터는 남아 있습니다. 공유 기기에서는 백업 후 전체 초기화를 이용하세요.');
      })}>로그아웃</button>
      <AccountSecurity user={user} onSignedOut={() => { setUser(null); setMode('login'); setRemote(undefined); setProviders([]); setShares([]); setMessage('비밀번호를 변경하고 모든 기기에서 로그아웃했습니다. 새 비밀번호로 로그인해 주세요.'); }} />
      <CloudSyncSettings userId={user.id} disabled={busy || checkingSession || storageBlocked} />
      ...
      <section className="account-section"><h3>읽기 전용 공유</h3>
        ...
        <button disabled={busy || checkingSession || storageBlocked || !shareTitle.trim()} className="button primary" onClick={() => void run(async () => {
          ...
          await accountApi('/api/shares', 'POST', { title: shareTitle.trim(), state });
          await refresh();
          setMessage('읽기 전용 공유 링크를 만들었습니다. 링크를 열어 내용을 확인할 수 있습니다.');
        })}>공유 링크 만들기</button>
        {shares.map((share) => <article className="share-row" key={share.id}><a target="_blank" rel="noreferrer" href={`/#share=${encodeURIComponent(share.token)}`}>{share.title}</a><button className="text-button danger-text" disabled={busy} onClick={() => void run(async () => { if (window.confirm('이 공유 링크를 폐기할까요?')) { await accountApi(`/api/shares/${share.id}`, 'DELETE'); await refresh(); } })}>폐기</button></article>)}
      </section>
      <section className="account-section"><h3>외부 캘린더 양방향 연동</h3>
        {scheduler?.error && <p className="form-error" role="status">자동 동기화 서버: {scheduler.error}</p>}
        ...
        {providers.map((provider) => <article className="integration-card" key={provider.id}>
          ...
          <p className="panel-description">{provider.recoveryRequired ? '연결 정보 복구 필요' : provider.connected ? `연결됨 · 마지막 동기화 ${provider.lastSyncedAt ?? '아직 없음'}` : provider.configured ? '연결할 수 있습니다.' : provider.configurationError ?? '서버에 제공자 앱 정보와 암호화 키를 설정해 주세요.'}</p>
          {provider.recoveryRequired && <p className="form-error" role="status">{provider.recoveryMessage ?? '저장된 연결 정보를 읽을 수 없습니다. 기존 서버 암호화 키를 복구하거나 연결 정보를 제거한 뒤 다시 연결해 주세요. 기존 일정은 유지됩니다.'}</p>}
          {provider.connected && provider.configurationError && <p className="panel-description">{provider.configurationError}</p>}
          {provider.lastError && <p className="form-error">마지막 동기화 오류: {provider.lastError}</p>}
          {provider.connected ? <div className="actions-wrap"><button className="button primary" disabled={busy || checkingSession || !provider.configured || provider.recoveryRequired} onClick={() => sync(provider.id)}>양방향 동기화</button><button className="button ghost" disabled={busy || checkingSession} onClick={() => void run(async () => { if (window.confirm(provider.recoveryRequired ? '이 제공자의 저장된 연결 정보를 제거할까요? 서버와 브라우저의 일정, 기존 외부 캘린더는 유지됩니다. 이후 직접 다시 연결해야 하며 다음 동기화에서 새 SHADOW 캘린더가 만들어집니다.' : '연결을 해제할까요? 외부 캘린더와 일정은 유지됩니다.')) { await accountApi(`/api/integrations/${provider.id}`, 'DELETE'); await refresh(); setMessage('연결 정보를 제거했습니다. 기존 일정은 유지됩니다. 서버 설정을 확인한 뒤 연결 버튼으로 다시 연결할 수 있습니다.'); } })}>{provider.recoveryRequired ? '연결 정보 제거' : '연결 해제'}</button></div>
          : provider.id === 'apple' ? <form className="stack" autoComplete="off" onSubmit={(event) => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); void run(async () => { if (!window.confirm('Apple iCloud에 연결할까요? 연결만으로 일정은 전송되지 않으며, 동기화할 때 새 전용 SHADOW 캘린더가 만들어집니다.')) return; await accountApi('/api/integrations/apple/connect', 'POST', { username: data.get('username'), password: data.get('password') }); form.reset(); await refresh(); }); }}>
          ...
          </form> : <button className="button ghost" disabled={busy || checkingSession || !provider.configured} onClick={() => void run(async () => { if (!window.confirm(`${names[provider.id]}에 연결할까요? 연결만으로 일정은 전송되지 않으며, 동기화할 때 새 전용 SHADOW 캘린더가 만들어집니다.`)) return; const { url } = await accountApi<{ url: string }>(`/api/integrations/${provider.id}/connect`, 'POST', {}); const target = new URL(url); if (target.protocol !== 'https:' || target.hostname !== authorizationHosts[provider.id] || target.username || target.password || target.port) throw new Error('올바르지 않은 인증 주소입니다.'); window.location.assign(url); })}>{names[provider.id]} 연결</button>}
          {provider.connected && <ProviderAutoSync provider={provider.id} userId={user.id} automation={provider.automation} disabled={busy || checkingSession || !provider.configured || !!provider.recoveryRequired} onUpdated={() => refresh()} />}
        </article>)}
        {conflicts.map((conflict) => <article className="form-conflicts" key={`${conflict.provider}-${conflict.id}`}><strong>{names[conflict.provider]} · {conflict.title}</strong><p>{conflict.reason}</p><div className="actions-wrap">{(['local', 'remote'] as const).map((choice) => <button disabled={busy || checkingSession} className="button ghost" key={choice} onClick={() => void run(async () => { await accountApi(`/api/integrations/${conflict.provider}/resolve`, 'POST', { conflictId: conflict.id, choice }); await refresh(); setMessage('선택한 버전으로 동기화를 실행했습니다. 남은 충돌을 확인하고 서버 데이터 가져오기로 이 브라우저에도 반영하세요.'); })}>{choice === 'local' ? 'SHADOW 서버 버전 사용' : '외부 버전 사용'}</button>)}</div></article>)}
      </section>
    </>}
